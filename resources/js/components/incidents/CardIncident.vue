<template>
    <v-card-text>
        ...
        <v-expansion-panels>
            <v-expansion-panel
                v-for="(incident, i) in incidentsStore.paginationIncidents"
                :key="i"
                elevation="1"
            >
                <v-expansion-panel-title>
                    <v-row no-gutters>
                        ...
                        >
                            {{ districtsStore.allDistricts.find(d => d.id === incident.district_id).title }} |
                            {{ humanizeDate(incident.created_at) }}
                        </v-col>
                        <v-spacer></v-spacer>
                        <v-col
                            cols="12"
                ...
                <v-expansion-panel-text>
                    ...
                    <v-row no-gutters class="mb-3">
                        ...
                        <v-col cols="12" sm="6" md="4">
                            {{ incident.user.name }} {{ incident.user.surname }}
                        </v-col>
                        <v-col cols="12" sm="6" md="4">
                            Тел. {{ incident.user.phone }}
                        </v-col>
                    </v-row>
                    <v-row no-gutters class="mb-3">
                        <v-col cols="12">
                    ...
                    <v-row>
                        ...
                        >
                            <MoveIncident
                                :object="incident"
                                @move-incidents="incidentsStore.getIncidents"
                            />
                            <EditIncident
                                :object="incident"
                                @get-incidents="incidentsStore.getIncidents"
                            />
                            <SoftDeleteIncident
                                :incident="incident"
                                @get-incidents="incidentsStore.getIncidents"
...
import MoveIncident from "@/components/incidents/MoveIncident.vue";
import EditIncident from "@/components/incidents/EditIncident.vue";
import SoftDeleteIncident from "@/components/incidents/SoftDeleteIncident.vue";
import DeleteIncident from "@/components/incidents/DeleteIncident.vue";
