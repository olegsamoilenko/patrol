<template>
    ...
    <v-btn
        v-if="authStore.user.roles.find((r) => r.name === 'Супер Адміністратор') ||
              ...
              authStore.checkPermission('Подія на головній редагувати')"
        icon
        color="green"
        class="mr-5"
        ...
        <v-card max-height="100vh">
            ...
            >
                <v-card
                    max-width="600px"
                >
                    <!--        TODO: Не подію, а щось типа відстежувану особу/завдання-->
                    <v-card-title>
    ...
    <v-dialog
        ...
        max-width="600px"
    >
<!--        TODO: Не подію відредагували, а щось типа відстежувану особу-->
        <v-card>
            <v-card-text>
                <span class="text-h5">
...
function prepareIncident(incident) {
    ...
    let url = `/api/edit-main-page-incident/${incident.id}`

    editObj(url, params, config);
